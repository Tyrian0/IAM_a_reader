...
def perform_experiment(**kwargs):
    ...
    if ctc_shortcut:
        output_shortcut = Conv1D(len(iam_dataset.char_to_num.get_vocabulary())+1, 3, activation='softmax', name='ctc_shortcut')(output_cnn)
    ...
    pd.DataFrame(history.history).plot(y=['loss', 'val_loss'], ax=ax)
